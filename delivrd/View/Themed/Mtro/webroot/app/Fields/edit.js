...
app.controller( 'EditField', ['$scope', '$http', '$modal', function ($scope, $http, $modal) {
    $scope.field = field;
    ...
    $scope.removeOption = function($id) {
        $http({
            method  : 'POST',
            url     : siteUrl+'fields/delete_option/',
            data    : 'id='+$id,
    ...
    $scope.addOption = function() {
        ...
        $http({
            method  : 'POST',
            url     : siteUrl+'fields/add_option/',
            data    : 'field_id='+$scope.field.Field.id+'&value='+$scope.new_option,
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).success(function(data) {
            if (data.action == 'success') {
                var pr = { 'FieldsValue': data.option };
