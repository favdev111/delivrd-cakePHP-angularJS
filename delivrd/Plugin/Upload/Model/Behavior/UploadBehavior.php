...
class UploadBehavior extends ModelBehavior {
    ...
    protected function _shouldSkip(Model $model, $field) {
        if (!empty($model->data[$model->alias]) && !in_array($field, array_keys($model->data[$model->alias]))) {
            return true;
        }
    ...
    protected function _resizeImagick(Model $model, $field, $path, $size, $geometry, $thumbnailPath) {
        ...
        $fileName = str_replace(
                array('{size}', '{geometry}', '{filename}', '{primaryKey}'), array($size, $geometry, $pathInfo['filename'], $model->id), $this->settings[$model->alias][$field]['thumbnailName']
        );
    ...
    protected function _pathThumbnail(Model $model, $field, $params = array()) {
        return str_replace(
                array('{size}', '{geometry}'), array($params['size'], $params['geometry']), $params['thumbnailPath']
        );
    }
    ...
    protected function _prepareFilesForDeletion(Model $model, $field, $data, $options = array()) {
        ...
        if (!empty($data[$model->alias][$field]) && !strlen($data[$model->alias][$field])) {
        ...
        foreach ($options['thumbnailSizes'] as $size => $geometry) {
            $fileName = str_replace(
                    array('{size}', '{geometry}', '{filename}', '{primaryKey}', '{time}', '{microtime}'), array($size, $geometry, $pathInfo['filename'], $model->id, time(), microtime()), $options['thumbnailName']
            );
